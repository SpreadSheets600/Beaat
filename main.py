...
class Autocatcher(commands.Bot):
    ...
    async def get_alternate_pokemon_name(self, name, languages=LANGUAGES):
        ...
        if pokemon:
            alternate_names = [
                alt_name for alt_name in pokemon.get("altnames", []) if alt_name.get("language").lower() in languages
            ]
            if alternate_names:
    ...
    async def on_message(message):
        ...
        if message.author.id == POKETWO_ID and message.channel.id in bot.whitelisted_channels:
            ...
            if "requesting a trade with" in message.content.lower():
                ...
                if message.components and message.components[0].children[0].label.lower() == "accept":
                    await time.sleep(random.choice(DELAY))
                    await message.components[0].children[0].click()
                logger.info("Trade Accepted")

            # Trade Confirmation
            if message.embeds and "are you sure you want to confirm this trade?" in message.embeds[0].description.lower():
                logger.info("Trade Confirmation Received")
                if message.components and message.components[0].children[0].label.lower() == "confirm":
                    await time.sleep(random.choice(DELAY))
                    await message.components[0].children[0].click()
        ...
        if message.embeds and "wild" in message.embeds[0].title.lower() and bot.verified:
        ...
        if "the pokémon is" in message.content.lower() and bot.verified:
            ...
            await message.channel.send("<@716390085896962058> c {}".format(solve(message.content)))
        ...
        if "congratulations" in message.content.lower() and bot.verified:
            ...
            if incense.lower() == "incense: active.":
                ...
                await send_log(embed=embed2, WEBHOOK_URL=WEBHOOK_URL) if embed2 else None

            await send_log(embed=embed1, WEBHOOK_URL=WEBHOOK_URL) if embed1 else None
